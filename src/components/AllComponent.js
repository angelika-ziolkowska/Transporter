...
export default class AllComponent extends React.Component {
    ...
    deleteTransporter(identityId){
        APIService.deleteTransporter(identityId).then( res => {
            this.setState({all: this.state.all.filter(transporter => transporter.identityId !== identityId)});
        });
    }
    ...
    render() {
        return (
            ...
            <div>
                <h2 className="text-center">All In Database</h2>
                <table className="table table-striped table-dark table-hover">
                    ...
                    <tbody>
                        {   Array.isArray(this.state.all) ? 
                        this.state.all.map(transporter => {return (
                            <tr key={transporter.identityId}>
                                ...
                                <td>
                                <button style={{ marginLeft: "10px" }} onClick={() => this.deleteTransporter(transporter.identityId)} className="btn btn-danger">Delete </button>
                                    {/* <button onClick={() => this.addBook(book)} className="btn btn-info">AddToDB</button> */
